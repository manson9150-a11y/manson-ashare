    ...
    def fetch(self, operation, accept=None, **kwargs):
        ...
        for i, adapter in enumerate(self.adapters):
            ...
            try:
                ...
                self.logs.append(SourceLog(source_name=adapter.source_name, operation=operation, success=False, fetched_at=datetime.now(TZ).isoformat(), reliability_level=adapter.reliability_level, fallback_priority=adapter.fallback_priority, is_fallback=i > 0, error=type(exc).__name__).model_dump())
