...
def sina_daily_frame(text, code, end, bars):
    """Decode only the quoted payload, never execute a remotely supplied script."""
    import quickjs
    code = symbol(code)
    if len(text) > 2_000_000:
        raise Unavailable('oversized daily payload')
    match = re.match(r'\s*var\s+KLC_K2_' + re.escape(code) + r'\s*=\s*"([A-Za-z0-9+/]+)"\s*;', text)
    if not match:
        raise Unavailable('unexpected daily payload or symbol mismatch')
    runtime = quickjs.Context()
    runtime.set_memory_limit(32 * 1024 * 1024)
    runtime.set_time_limit(2)
    runtime.set_max_stack_size(512 * 1024)
    runtime.eval((Path(__file__).parent / 'vendor/sina_klc.js').read_text())
    rows = json.loads(runtime.get('d')(match.group(1)).json())
    records = []
    for item in rows:
        day = str(item.get('date', ''))[:10]
        if not day or day > str(end):
            continue
        records.append({'timestamp': datetime.fromisoformat(day + 'T15:00:00').replace(tzinfo=TZ),
            **{key: number(item.get(key)) for key in ('open', 'close', 'high', 'low', 'volume', 'amount')},
            'source': 'sina', 'adjustment': 'none'})
...
class EastmoneyAdapter(Adapter):
    ...
    def fetch(self, operation, **kwargs):
        ...
        if operation in ('history', 'index_history'):
            sym = symbol(kwargs['code'])
            bars = kwargs.get('bars', self.config['sources']['history_bars'])
            payload = self.get('https://push2his.eastmoney.com/api/qt/stock/kline/get', params={'secid': ('1' if sym.startswith('sh') else '0') + '.' + sym[2:], 'klt': 101, 'fqt': 0, 'beg': '0', 'end': kwargs['end'].strftime('%Y%m%d'), 'lmt': bars, 'fields1': 'f1,f2,f3,f4,f5,f6', 'fields2': 'f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61'}).json().get('data')
            if not payload:
                raise Unavailable('no history')
            if str(payload.get('code')) != sym[2:]:
                raise Unavailable('history symbol mismatch')
            return bar_frame([line.split(',')[:7] for line in payload['klines'][-bars:]], self.source_name)
        ...
        if operation == 'broken_pool':
            data = self.get('https://push2ex.eastmoney.com/getTopicZBPool', params={'ut': '7eea3edcaed734bea9cbfc24409ed989', 'dpt': 'wz.ztzt', 'Pageindex': 0, 'pagesize': 1000, 'sort': 'fbt:asc', 'date': kwargs['day'].strftime('%Y%m%d')}).json().get('data')
            if not isinstance(data, dict) or not isinstance(data.get('pool'), list) or not data['pool']:
                raise Unavailable('broken pool empty or unavailable; zero not verified')
            if str(data.get('qdate')) != kwargs['day'].strftime('%Y%m%d'):
                raise Unavailable('broken pool date mismatch')
            if int(data.get('tc', len(data['pool']))) > len(data['pool']):
                raise Unavailable('broken pool truncated')
            return [{'code': r['c'], 'break_count': r.get('zbc'), 'source': self.source_name, 'trade_date': str(kwargs['day'])} for r in data['pool']]
...
class TencentAdapter(Adapter):
    ...
    def fetch(self, operation, **kwargs):
        ...
        if operation in ('history', 'index_history'):
            ...
            bars = kwargs.get('bars', self.config['sources']['history_bars'])
            data = self.get('https://web.ifzq.gtimg.cn/appstock/app/fqkline/get', params={'param': f'{code},day,,{kwargs["end"]:%Y-%m-%d},{bars},'}).json()
...
class SinaAdapter(Adapter):
    ...
    def fetch(self, operation, **kwargs):
        if operation in ('history', 'index_history'):
            code = symbol(kwargs['code'])
            payload = self.get(f'https://finance.sina.com.cn/realstock/company/{code}/hisdata_klc2/klc_kl.js').text
            return sina_daily_frame(payload, code, kwargs['end'], kwargs.get('bars', self.config['sources']['history_bars']))
        if operation == 'universe':
            rows = []
