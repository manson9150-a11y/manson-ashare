...
def analyze_stocks(quotes,factors,sectors,membership,events,market,config,limit_pool=None):
    ...
    for q in quotes:
        ...
        position,reason=classify(f,sector,config,hard)
        ...
        stocks.append({**q.model_dump(mode='json'),**f,'stock_code':q.code,'stock_name':q.name,'sector':sector['name'],'sector_id':sector['id'],'sector_score':sector['score'],'sector_state':sector['state'],'market_score':market.get('score'),'position_type':position,'atr_state':atr_state,'atr_risk':penalty,'risk_score':penalty,'risk':'高' if hard or penalty>=r['high_threshold'] else '中' if penalty>=r['medium_threshold'] else '低','hard_risk':hard,'total_score':round(score,2) if score is not None else None,'score_coverage':round(coverage,2),'scores':components,'tradability_score':0 if position in ('排除','高位观察') else round(score or 0,2),'subjective_probability':None,'probability_basis':'未校准，不输出伪精确概率','deep_score':None,'deep_components':{k:None for k in config['scoring']['deep_weights']},'board_count':limit.get('board_count'),'is_limit_up':bool(is_limit),'limit_details':limit,'volume_type':volume_type,'events':ev,'independent_catalyst':bool(independent_events),'catalyst_score':max((e['impact_score'] for e in independent_events),default=None),'expectation_score':None,'positive_factor':reason if position!='排除' else '无已确认优势','negative_factor':reason if position in ('排除','高位观察') else ('未读取完整公告正文；需人工核验' if ev else '独立催化与龙虎榜尚未确认'),'validation_condition':f"板块维持扩散，价格守住MA20（{f.get('ma20') or '待补'}），ATR不过热",'invalidation_condition':'板块转弱、放量跌破MA20或出现重大风险公告','exclusion_reason':reason if position=='排除' else None,'rank_history':[]})
    return stocks
