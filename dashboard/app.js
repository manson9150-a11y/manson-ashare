...
function showDetail(code){
 ...
 const fields=[['现价',s.price],['MA5',s.ma5],['MA10',s.ma10],['MA20',s.ma20],['MA60',s.ma60],['ATR14',s.atr],['NATR %',s.natr],['MoveATR',s.move_atr],['供应商板数',s.board_count],['日线推导板数',s.derived_board_count],['板块分',s.sector_score],['交易性',s.tradability_score],['量比',s.volume_ratio],['换手率 %',s.turnover]];
 $('detail-body').innerHTML=`<h2>${esc(s.stock_name)} <small>${esc(code)}</small></h2><p>${esc(s.sector)} · ${esc(s.position_type)} · ${esc(s.atr_state)} ATR · ${esc(s.risk)}风险<br>行情时间 ${esc(s.timestamp)} · 来源 ${esc(s.source)} · 价格序列未经复权</p><div class="detail-grid">${fields.map(([k,v])=>`<div>${esc(k)}<b>${num(v)}</b></div>`).join('')}</div>${chart(s.bars)}<div class="detail-columns"><div><h3 class="detail-heading">最关键正因子</h3><p>${esc(s.positive_factor)}</p><h3 class="detail-heading">验证条件</h3><p>${esc(s.validation_condition)}</p></div><div><h3 class="detail-heading">最关键负因子</h3><p>${esc(s.negative_factor)}</p><h3 class="detail-heading">失效条件</h3><p>${esc(s.invalidation_condition)}</p></div></div><p>涨停身份：${esc(({vendor_limit_price:'供应商明确价格证据',rule_derived_not_vendor_confirmed:'价格规则推导，待交叉核验'})[s.limit_identity_basis]||'未记录')}；供应商板数与日线推导分别展示，推导未覆盖全部特殊交易及除权情形。</p><p>量价：${esc(s.volume_type)}。主观上涨/连板概率：${s.subjective_probability==null?'未输出（无已校准模型）':`${num(s.subjective_probability*100,0)}%（AI未校准）`}。交易性分：${num(s.tradability_score,0)}。100分深度评分：${num(s.deep_score)}。</p><h3 class="detail-heading">事件与催化</h3>${(s.events||[]).map(e=>`<div class="event"><a href="${safeURL(e.url)}" target="_blank" rel="noopener noreferrer">${esc(e.title)} ↗</a><br><small>${esc(e.source)} · ${esc(e.publish_time_precision==='date'?e.publish_time.slice(0,10)+'（仅日期，无精确发布时间）':e.publish_time)} · ${esc(e.expectation||'证据不足')} · ${e.verified?'已核验':'待正文核验'}</small>${e.verification?`<p>${e.verified?'正文量化事实核验通过':'正文证据尚未满足核验要求'}<br>${esc(e.verification.evidence||'')}<br>${esc(e.verification.score_basis||'')}</p>`:''}</div>`).join('')||'<p>无已确认事件；不按新闻条数加分。</p>'}<h3 class="detail-heading">历史排名</h3><p>${(s.rank_history||[]).map(r=>`${esc(r.date)} ${esc(r.stage)} #${r.rank}（${num(r.score)}）`).join(' → ')||'首次观察，等待后续验证。'}</p>`;
 $('detail').showModal();
}
