...
一个无需常驻服务器的个人 A 股个股研究台，聚焦短期强势、独立催化、条件入场与高波动观察。市场全景和连板天梯通过悟道查看；本站保留少量个股的判断、失效条件与后续变化。

**2026-09-12 个股研究改版：取消主动连板池与重复市场榜单，增加独立于板块映射的个股采集通道。公告先给分析结论，原文折叠为依据。价格计划、历史补充和自动分析边界见 [新版说明](docs/STOCK_FOCUS.md)。以下早期模块说明保留用于理解历史结果，以新版说明为准。**

**2026-09-10 更新：交易日三次研究（08:00 / 12:00 / 22:00），补充新浪成交额日线与 BaoStock 备用源，新增动态因子能力表。旧报告按原阶段保留；新数据由后续正式任务生成。**

正式工作台：https://manson9150-a11y.github.io/manson-ashare/ 。上线证据见 [部署记录](docs/DEPLOYMENT_CHECKLIST.md)。

2026-09-08：已修复A/C池数据链路、独立公告发现、官方正文量化核验与空池诊断，见[修复说明](docs/CANDIDATE_POOL_REPAIR.md)。旧快照保持冻结，新规则用于部署后的新任务。

## 1. 它怎样运行

```text
GitHub Actions 三个时间节点
  → 免费公开 Adapter + 独立备用源
  → 时间戳 / 交易日 / 缺失 / 异常 / 冲突检查
...
首页展示个股研究卡、催化研判、跟踪变化和数据依据。可按短期强势、独立催化、高波动筛选，搜索名称、代码或题材；详情包含条件区间、失效参考和日线依据。高波动仅观察，不生成追涨买点。
...
|北京时间|任务|唯一正式前序|输出|
|---|---|---|---|
|08:00|盘前计划|前交易日22:00|合并隔夜确认与盘前终审，最多10只；刷新公告及风险|
|12:00|午间验证|当天08:00|使用11:30上午收盘行情，验证早盘名单，最多5只|
|22:00|晚间复盘|独立完整扫描|合并收盘扫描与晚间二筛，建立次日A/B/C观察池|

08:00沿用前收盘行情与市场分；外部市场变量尚未纳入评分。12:00不把半日成交额与全日均值比较。22:00不依赖已经取消的16:00阶段。

首次启动从下一个有效交易日22:00建立完整链。迁移时，08:00允许读取前交易日已冻结的21:30（次选16:00）报告；12:00允许承接当天已冻结的08:30报告。正常前序若已存在但失败，不绕过失败记录。不会用现在的行情倒填错过的早报或午报。

UTC cron为00:00 / 04:00 / 14:00；GitHub Actions可能排队延迟。程序再次检查北京时间、交易日历及阶段窗口（08:00–09:10、12:00–12:45、22:00–23:59）。旧五阶段快照及历史周末初始化保持原样，自动调度只保留三次。

详见 [三次调度与因子能力升级](docs/THREE_STAGE_UPGRADE.md)。
...
5. `Daily A-share Research` 已预置三个定时任务；上传到默认分支后运行。
...
正式研究日志使用你的一份现有Google文档，按日期增加一章，每章预留三个阶段；同日若已存在旧五阶段章节，会追加新占位而保留旧内容。已经找到的现有文档可优先使用“**A股短线全景研究档案**”；文档ID不写进公开仓库。
...
|MISSING_PREDECESSOR|正式上一阶段不存在|从下一次22:00重新建立闭环|
...
模拟会先生成前一日22:00，再完整执行当日三阶段，输出在 `artifacts/demo/`。它只更新 `dashboard/data/demo.json`，不写正式历史或Google Docs，不是数据源fallback。
...
接口字段依据[AKShare公开源码和接口目录](https://github.com/akfamily/akshare)，本项目直接调用受控HTTP适配器；**当前没有把AkShare本身作为独立备用源**，因为它常与同一上游共用故障。QuickTiny仅提供禁用适配器，等待公开可用性和条款核验。巨潮官方PDF已接入有限类型的量化事实核验，其他公告类型、财联社和海外行情仍待扩充。
...
每阶段保留因子Parquet、候选JSON、阶段结果和Markdown。22:00额外保存压缩的全市场每日快照，用于真实结果追踪。仅在最新滚动缓存保留候选最多65根日线用于图表，历史阶段JSON不重复保存K线；不提交分钟原始数据。
...
收盘复盘接受目标交易日16:00之后的真实收盘行情更新，保留来源时间，不把后续交易日的价格混入；这与重建“16:00当时可知信息”的时点回测不同。实时阶段仍保留各自的时间护栏。复盘采用经用户同意的事后行业分类，覆盖47.4%；250只日线中195只缺少成交额历史，历史公告风险也未完整核验，因此不能视为全市场完整回测。
...
本次历史复盘已单独补写主Google文档的“历史复盘归档（补写）”章节；历史复盘命令仍不会自动写入正式阶段占位。正式任务完成时，网页更新与Google文档归档并行使用；文档写入失败保留待补传队列。
...
scripts/            真实源探测与浏览器验收
tests/              可脱网运行的单元与集成测试
.github/workflows/  三时点流水线、CI、静态发布
```
...
已为三个正式研究阶段接入悟道题材强度、行业涨幅与前十题材成分。云端需配置 `WUDAO_API_KEY` Actions Secret。首页“市场热点”为悟道原始榜单，“已覆盖成分规则评分”为本系统规则评分，两种口径分别标注。日期、限流、筛选参与方式和手工验收见 `docs/DATA_SOURCES.md`。密钥不得写入源码、报告或前端。
