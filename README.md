...
首次启动另支持显式周末初始化：用户要求后，于实际周末时点用已完成的收盘复盘检查原候选并尝试刷新事件，保存到 `data/bootstrap/<目标交易日>.json`。只有指定交易日07:30且正式前序完全不存在时可使用；失败的正式前序不会被覆盖，后续阶段保留初始化来源和公告缺口，不计入常规因子效果统计。2026-09-07的初始化已于2026-09-06建立，包含30只观察候选；当次公告接口不可用，明早再次请求。正式收盘扫描完成后建立新的正常数据链。
...
|公告事件|东方财富公告结构化入口|手工核验事件 / 前序已知事件|标题只作风险预警；不自动给正催化打满分|
