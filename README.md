...
|08:30|盘前终审|当天07:30|最多10只；A/B沿用前序，C可新增已核验事件|
...
已为五个正式研究阶段接入悟道题材强度、行业涨幅与前十题材成分。云端需配置 `WUDAO_API_KEY` Actions Secret。首页“市场热点”为悟道原始榜单，“已覆盖成分规则评分”为本系统规则评分，两种口径分别标注。日期、限流、筛选参与方式和手工验收见 `docs/DATA_SOURCES.md`。密钥不得写入源码、报告或前端。
